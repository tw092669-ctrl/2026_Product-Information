...
export function getBrandDisplayName(brand: string): string {
  ...
  if (name === '三菱重工' || name.includes('重工')) return '重工';
  return name;
}
...
export function getBrandColor(brand: string, isLightBg: boolean = false): string {
  ...
  if (name.includes('三菱') || name.includes('重工')) return isLightBg ? 'bg-rose-100 text-rose-800' : 'bg-rose-500/20 text-rose-400 border border-rose-500/30';
